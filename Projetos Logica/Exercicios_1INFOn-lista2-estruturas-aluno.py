...
def aumento_salarial(salario, porcentagem):
    ...
    o novo salário"""
    aumento_salarial = (salario * porcentagem) / 100 + salario
    aumento_salarial = round (aumento_salarial, 2)
...
def preco_com_desconto(preco_original, percentual_desconto):
    ...
    percentual_desconto = preco_original - preco_com_desconto
    return round (percentual_desconto,2)
...
def dias_para_segundos(dias, horas, minutos, segundos):
    ...
    a data em segundos"""
    dat_em_segundos = segundos + (minutos * 60) + (horas * 3600) + (dias * 86400)
    return dat_em_segundos
...
def dois_elevado_a_um_milhao():
    ...
    algarismos"""
    dois_elevado_a_um_milhão = 2**1000000
    result = str(dois_elevado_a_um_milhão)
    return len(result)
